...
public class GoldStorage extends Building {
    ...
    public static ItemStack getShopItem() {
        return GUIHelper.attachLore(BuildingGlobals.GOLD_STORAGE_ITEM_STACK.clone(), Arrays.asList(
                ...
                ChatColor.GRAY + "Cost: " + BuildingGlobals.GOLD_STORAGE_COST[0] + " " + Globals.ELIXIR_DISPLAY_NAME,
                ChatColor.GRAY + "Build Time: " + Globals.timeFromSeconds(BuildingGlobals.GOLD_STORAGE_BUILD_TIME[0])));
    ...
    public List<String> getUpgradeDescription() {
        ...
        int level = getLevel();
        String currency = " " + Globals.GOLD_DISPLAY_NAME.toLowerCase() + " ";
        lore.add(ChatColor.GRAY + " + " + ChatColor.GREEN + (BuildingGlobals.GOLD_STORAGE_CAPACITY[level] -
                BuildingGlobals.GOLD_STORAGE_CAPACITY[level - 1] ) + currency
                + ChatColor.GRAY + "storage capacity");
        lore.add(ChatColor.GRAY + " + " + ChatColor.GREEN + (BuildingGlobals.GOLD_STORAGE_HITPOINTS[level] -
                BuildingGlobals.GOLD_STORAGE_HITPOINTS[level - 1]) + ChatColor.GRAY + " hitpoints");
        lore.add(ChatColor.GRAY + "Cost: " + BuildingGlobals.GOLD_STORAGE_COST[level] + " " + Globals.ELIXIR_DISPLAY_NAME);
        lore.add(ChatColor.GRAY + "Upgrade Time: " + Globals.timeFromSeconds(BuildingGlobals.GOLD_STORAGE_BUILD_TIME[level]));
        return lore;
    }
    @Override
    public ItemStack getStatsItem() {
        ...
        return GUIHelper.attachNameAndLore(stack,getDisplayName() + " Stats", Arrays.asList(
                ChatColor.GRAY + " " + (int) getStorageCapacity("gold") + currency + ChatColor.GRAY + "storage capacity",
